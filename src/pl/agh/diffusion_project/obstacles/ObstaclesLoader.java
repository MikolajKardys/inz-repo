...
public class ObstaclesLoader {
    ...
    private ObstaclesLoader(int imageX, int imageY){
        this.terrainX = imageX;
        this.terrainY = imageY;
        this.heightMatrix = new int[imageX][imageY];
    }

    public static ObstaclesLoader loadObstaclesFromBitmap(String fileName, int blockSize) throws IOException {
        BufferedImage img = ImageIO.read(new File(fileName));

        ObstaclesLoader loader = new ObstaclesLoader(img.getWidth(), img.getHeight());
    ...
    public static ObstaclesLoader loadObstaclesFromBitmap(String fileName) throws IOException {
        return loadObstaclesFromBitmap(fileName, 1);
    }
